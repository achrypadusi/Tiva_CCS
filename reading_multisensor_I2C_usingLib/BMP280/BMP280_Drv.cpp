 ...
 */
Bmp280ErrCode bmp280_create_predefined_settings(
        bmp280 *sensor, const Bmp280MeasureSettings settings)
{
    switch (settings)
    {
    case HandLow:
 ...
 *
 */
Bmp280ErrCode bmp280_init(bmp280 *sensor, const Bmp280ComProtocol protocol,
...
Bmp280ErrCode bmp280_get_status(bmp280 *sensor)
{
    ...
    bit_get(statusReturn, BMP280_MEASURING_MASK) ?
            (sensor->lastKnowStatus.isMeasuring = true) :
            (sensor->lastKnowStatus.isMeasuring = false);
    bit_get(statusReturn, BMP280_UPDATING_MASK) ?
            (sensor->lastKnowStatus.isUpdating = true) :
 ...
 */
Bmp280ErrCode bmp280_get_temp_press(bmp280 *sensor, float *temperatureC,
                                    float *pressPa, Bmp280CalibParam calibParam)
{
    BMP280_TRY_FUNC(bmp280_port_prep(sensor));
    uint8_t rawData[RAW_TEM_TOTAL_BYTE + RAW_PRESS_TOTAL_BYTE + 3];
    bmp280_get_register(sensor, BMP280_BASEADDR + Press_msb, rawData,
                        RAW_TEM_TOTAL_BYTE + RAW_PRESS_TOTAL_BYTE + 3);
    int32_t rawPress = (int32_t) ((((uint32_t) (rawData[0])) << 12)
            | (((uint32_t) (rawData[1])) << 4) | ((uint32_t) rawData[2] >> 4));

    int32_t rawTemp = (int32_t) ((((int32_t) (rawData[3])) << 12)
            | (((int32_t) (rawData[4])) << 4) | (((int32_t) (rawData[5])) >> 4));
    *temperatureC = (float) bmp280_compensate_T_int32(rawTemp, &calibParam)
            * 0.01;
 ...
 *
 */
Bmp280ErrCode bmp280_get_calibration_data(bmp280 *sensor,
                                          Bmp280CalibParam *calibParam)
{
    ...
    bmp280_get_register(sensor, BMP280_CALIB_START_ADDR, rawCalibData,
                        BMP280_CALIB_DATA_SIZE + 5);
